...
class CustomerSubscriptionRepositoryIT extends PersistenceITBase {
    ...
    void setUp() {
        userA = users.save(UserEntity.builder()
                .email("a-" + UUID.randomUUID() + "@nx.local")
                .role(UserRole.USER)
                .active(true)
                .googleLinked(false)
                .build());
        userB = users.save(UserEntity.builder()
                .email("b-" + UUID.randomUUID() + "@nx.local")
                .role(UserRole.USER)
                .active(true)
                .googleLinked(false)
                .build());
        plan = plans.save(SubscriptionPlanEntity.builder()
                .code("pro-" + UUID.randomUUID())
                .name("Pro")
    ...
    void findByUserId_returnsOnlyOwnerRows() {
        ...
        assertThat(ofA).hasSize(2)
                .allMatch(s -> s.getUser().getId().equals(userA.getId()));
    }

    @Test
    void findByStripeSubscriptionId_hitAndMiss() {
        subscriptions.save(sub(userA, SubscriptionStatus.ACTIVE, "sub_stripe_xyz",
    ...
    void findActiveByUserId_filtersByStatusAndOrdersByPeriodEndDesc() {
        ...
        subscriptions.save(sub(userA, SubscriptionStatus.CANCELED, "sub_canceled",
                Instant.parse("2026-09-01T00:00:00Z")));
        subscriptions.save(sub(userA, SubscriptionStatus.PAST_DUE, "sub_pastdue",
        ...
        assertThat(active).extracting(CustomerSubscriptionEntity::getStripeSubscriptionId)
                .containsExactly("sub_trial", "sub_active");
    }
}
