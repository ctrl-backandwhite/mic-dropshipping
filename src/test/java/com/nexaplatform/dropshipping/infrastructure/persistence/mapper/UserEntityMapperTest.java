...
class UserEntityMapperTest {
    ...
    void roundTrip_preservesScalarFields() {
        User source = User.builder().id(UUID.randomUUID()).email("user@nx036.local").passwordHash("HASH")
                .role(UserRole.OPERATOR).active(true).activationCode("ACT-1")
                .activationCodeExpiresAt(Instant.parse("2026-02-01T00:00:00Z")).failedLoginCount(2)
                .lockedUntil(Instant.parse("2026-02-02T00:00:00Z")).lastLogin(Instant.parse("2026-02-03T00:00:00Z"))
                .displayName("Jane").companyName("Acme").country("ES").phone("600100200").avatarUrl("/a.png")
                .language("es").googleLinked(true).build();
        ...
        assertThat(result).usingRecursiveComparison().ignoringFields(
                ...
                "totp", "resetTokens", "totpEnabled").isEqualTo(source);
    }

    @Test
    void toEntity_ignoresAudit() {
        User source = User.builder().id(UUID.randomUUID()).email("audit@nx036.local").role(UserRole.ADMIN).active(true)
                .createdAt(Instant.now()).updatedAt(Instant.now()).createdBy("creator").updatedBy("editor").build();
