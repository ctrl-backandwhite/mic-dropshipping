...
class ProductMapperTest {
    ...
    void setup() {
        ...
        when(marginService.apply(Mockito.any(), Mockito.any(), Mockito.any()))
                .thenAnswer(inv -> new MarginService.PriceWithMargin(
                        inv.getArgument(0), inv.getArgument(0), null, BigDecimal.ZERO));
    ...
    void summary_picks_es_title_when_available() {
        ProductEntity p = ProductEntity.builder()
                .source("1688").externalId("X")
                .titleZh("中文标题")
                .basePrice(new BigDecimal("9.99")).currency("CNY")
                .monthlySales(10)
                .build();
        ProductTranslationEntity tr = ProductTranslationEntity.builder()
                .product(p).language("es").title("Título").build();
    ...
    void summary_falls_back_to_zh_when_translation_missing() {
        ProductEntity p = ProductEntity.builder()
                .titleZh("Fallback ZH").basePrice(new BigDecimal("1")).build();
        p.setTranslations(new ArrayList<>());
        p.setImages(new ArrayList<>());
    ...
    void image_url_prefers_cdn_over_source() {
        ...
        ProductImageEntity img = ProductImageEntity.builder()
                .sourceUrl("https://cbu01.alicdn.com/x.jpg")
                .cdnUrl("https://cdn.nexadrop.io/x.webp")
                .position(0)
                .build();
