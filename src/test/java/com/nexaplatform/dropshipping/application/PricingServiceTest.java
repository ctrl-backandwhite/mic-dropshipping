...
class PricingServiceTest {
    ...
    void priceFor_convertsCnyToUsdAndAppliesMargin_thenDisplaysInUsd() {
        ...
        when(marginService.apply(any(), any(), any()))
                .thenReturn(new PriceWithMargin(new BigDecimal("14.00"), new BigDecimal("21.00"), null, new BigDecimal("50.0")));
    ...
    void priceFor_variantPriceOverridesProductBase() {
        ...
        when(marginService.apply(any(), any(), any()))
                .thenReturn(new PriceWithMargin(new BigDecimal("7.50"), new BigDecimal("12.00"), null, new BigDecimal("60.0")));
        when(currencyService.usdToDisplay(any(BigDecimal.class)))
                .thenAnswer(inv -> inv.getArgument(0));
        when(currencyService.symbolOf(anyString())).thenReturn("$");

        ProductEntity p = ProductEntity.builder()
                .basePrice(new BigDecimal("100.00")).currency("CNY").build();
    ...
    void priceFor_handlesNullBasePrice_returnsNullCost() {
        when(marginService.apply(any(), any(), any()))
                .thenReturn(new PriceWithMargin(null, null, null, null));
    ...
    void priceFor_displayCurrencyFollowsCurrencyHolder() {
        ...
        when(marginService.apply(any(), any(), any()))
                .thenReturn(new PriceWithMargin(new BigDecimal("10.00"), new BigDecimal("15.00"), null, new BigDecimal("50.0")));
        ...
        ProductEntity p = ProductEntity.builder()
                .basePrice(new BigDecimal("71.40")).currency("CNY").build();
    ...
    void priceFor_zeroSupplierPrice_returnsZeroAcrossPipeline() {
        ...
        ProductEntity p = ProductEntity.builder()
                .basePrice(BigDecimal.ZERO).currency("CNY").build();
    ...
    void priceFor_convertsToEur_appliesExchangeRate() {
        ...
        when(marginService.apply(any(), any(), any()))
                .thenReturn(new PriceWithMargin(new BigDecimal("100.00"), new BigDecimal("150.00"), null, new BigDecimal("50.0")));
        ...
        ProductEntity p = ProductEntity.builder()
                .basePrice(new BigDecimal("700.00")).currency("CNY").build();
    ...
    void priceFor_staleExchangeRate_stillProducesValidPrice() {
        ...
        when(currencyService.toUsd(any(BigDecimal.class), eq("CNY")))
                .thenReturn(new BigDecimal("13.50")); // valor "rancio" pero válido
        when(marginService.apply(any(), any(), any()))
                .thenReturn(new PriceWithMargin(new BigDecimal("13.50"), new BigDecimal("20.25"), null, new BigDecimal("50.0")));
        when(currencyService.usdToDisplay(any())).thenAnswer(inv -> inv.getArgument(0));
        when(currencyService.symbolOf(anyString())).thenReturn("$");
    ...
    void priceFor_tieredPricing_minMaxBoundaries() {
        ...
        when(currencyService.toUsd(new BigDecimal("50.00"), "CNY"))
                .thenReturn(new BigDecimal("7.00")); // min
        when(currencyService.toUsd(new BigDecimal("1000.00"), "CNY"))
                .thenReturn(new BigDecimal("140.00")); // max
        when(marginService.apply(any(), any(), any()))
                .thenAnswer(inv -> {
        ...
        PricedAmount min = service.priceFor(ProductEntity.builder().basePrice(new BigDecimal("50.00")).currency("CNY").build());
        PricedAmount max = service.priceFor(ProductEntity.builder().basePrice(new BigDecimal("1000.00")).currency("CNY").build());
    ...
    void priceFor_defaultsCurrencyToCnyWhenMissing() {
        ...
        when(marginService.apply(any(), any(), any()))
                .thenReturn(new PriceWithMargin(new BigDecimal("5.00"), new BigDecimal("8.00"), null, new BigDecimal("60.0")));
        when(currencyService.usdToDisplay(any())).thenAnswer(inv -> inv.getArgument(0));
        when(currencyService.symbolOf(anyString())).thenReturn("$");

        ProductEntity p = ProductEntity.builder()
                .basePrice(new BigDecimal("35.00"))
                .currency(null) // explicit null
                .build();
