...
public class OpenApiConfig {
    ...
    public OpenAPI nexaDropOpenAPI() {
        ...
        api.addExtension("x-rate-limit", Map.of(
                "policies", List.of(
                ...
                "headers", List.of("RateLimit-Limit", "RateLimit-Remaining", "RateLimit-Reset", "Retry-After")));
        return api;
    }
    ...
    private OpenAPI buildOpenAPI() {
        OAuthFlow clientCredentials = new OAuthFlow()
                ...
                .scopes(new Scopes()
                        .addString("catalog.read",  "Read products, categories, suppliers and pricing tiers")
                        .addString("orders.write",  "Create and read partner orders, request shipping quotes")
                        .addString("shop.sync",     "Push catalog listings and receive sync callbacks for connected shops"));

        SecurityScheme oauth2 = new SecurityScheme()
                .type(SecurityScheme.Type.OAUTH2)
                .description("OAuth2 client_credentials. Use the /oauth2/token endpoint with your `client_id` and `client_secret` to obtain a JWT.")
        ...
        SecurityScheme session = new SecurityScheme()
                .type(SecurityScheme.Type.APIKEY)
                .in(SecurityScheme.In.COOKIE)
                .name("SESSION")
                .description("HTTP session cookie issued by the web login. Used for /api/admin/** and /api/me/**.");

        SecurityScheme webhookHmac = new SecurityScheme()
                .type(SecurityScheme.Type.APIKEY)
                .in(SecurityScheme.In.HEADER)
                .name("X-NX-Signature")
                .description("HMAC-SHA256 of the raw request body, hex-encoded, signed with the shop connection secret. Verified on inbound order webhooks.");

        return new OpenAPI()
                .info(new Info()
                        .title("NexaDrop Platform API")
                        .version("1.1.0")
                        .description("""
                                ...
                                **Idempotency** — write endpoints (`POST` orders, listings, sync) honor the `Idempotency-Key` header for up to 24 h.
                                """)
                        .contact(new Contact().name("NexaDrop API").email("api@nexadrop.io").url("https://nexadrop.io/developers"))
                        .license(new License().name("Proprietary").url("https://nexadrop.io/terms")))
                .servers(List.of(
                        new Server().url(publicBase).description("Current environment"),
                ...
                .tags(List.of(
                        new Tag().name("Auth").description("OAuth2 token issuance and OIDC discovery"),
                        ...
                        new Tag().name("Admin · Partners").description("Manage OAuth clients, webhooks and apps (session)"),
                ...
                .components(new Components()
                        ...
                        .addSecuritySchemes("session", session)
                        .addSecuritySchemes("webhookHmac", webhookHmac));
    }
    ...
    public GroupedOpenApi storefrontApi() {
        return GroupedOpenApi.builder()
                .group("storefront")
                .pathsToMatch("/api/v1/storefront/**")
                .addOpenApiCustomizer(globalResponses())
                .build();
    }
    ...
    public GroupedOpenApi adminApi() {
        return GroupedOpenApi.builder()
                ...
                .addOpenApiCustomizer(applyDefaultSecurity("session"))
                .addOpenApiCustomizer(globalResponses())
                .build();
    }
    ...
    private OpenApiCustomizer applyDefaultSecurity(String... schemeNames) {
        return openApi -> {
            ...
            for (String name : schemeNames) requirement.addList(name);
            if (openApi.getPaths() == null) return;
            openApi.getPaths().values().forEach(path -> path.readOperationsMap().forEach((method, op) -> {
                if (op.getSecurity() == null || op.getSecurity().isEmpty()) op.addSecurityItem(requirement);
            }));
