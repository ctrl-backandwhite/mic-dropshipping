...
public interface PartnerPaymentApi {
    ...
    ResponseEntity<List<OrderPaymentDtoOut>> list(@AuthenticationPrincipal Jwt jwt, @PathVariable UUID orderId);

    @Operation(summary = "Read a payment by id (poll while pending)")
    @GetMapping("/{orderId}/payments/{paymentId}")
    ResponseEntity<OrderPaymentDtoOut> get(@AuthenticationPrincipal Jwt jwt, @PathVariable UUID orderId,
