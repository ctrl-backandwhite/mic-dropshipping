...
public interface InboundShopWebhookApi {
    ...
    @Operation(
            summary = "Inbound order from a connected shop",
            description = "Your e-commerce posts the customer's order here. NexaDrop verifies the HMAC, "
                    + "maps the payload to its canonical order shape and creates the dropship order. "
                    + "Replay-safe via the `Idempotency-Key` header (24 h window).",
    ...
    ResponseEntity<OrderView> receiveOrder(
            @PathVariable UUID shopId,
            @Parameter(description = "Hex HMAC-SHA256 of the raw body, signed with the shop connection's inbound secret.",
                    required = true)
            @RequestHeader("X-NX-Signature") String signature,
            @Parameter(description = "Optional external order id used for deduplication (24 h window).")
