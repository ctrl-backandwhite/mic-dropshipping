...
public class PayPalGateway implements PaymentGateway {
    ...
    @Value("${nexadrop.paypal.enabled:false}")    private boolean enabled;
    @Value("${nexadrop.paypal.client-id:}")        private String clientId;
    @Value("${nexadrop.paypal.client-secret:}")    private String clientSecret;
    @Value("${nexadrop.paypal.base-url:https://api-m.sandbox.paypal.com}") private String baseUrl;
    @Value("${nexadrop.paypal.return-url:http://localhost:3003/wallet/paypal-return}") private String returnUrl;
    @Value("${nexadrop.paypal.cancel-url:http://localhost:3003/wallet/recharge?cancelled=1}") private String cancelUrl;
    @Value("${nexadrop.paypal.platform-id:nexadrop-dropshipping}") private String platformId;
    @Value("${nexadrop.paypal.platform-env:dev}") private String platformEnv;

    private final WebClient.Builder webClientBuilder;
    public PayPalGateway(WebClient.Builder b) { this.webClientBuilder = b; }

    @Override public boolean supports(PaymentMethod m) { return m == PaymentMethod.PAYPAL; }
    @Override public String providerName() { return "paypal"; }
    ...
    public InitiateResult initiate(PaymentEntity p) {
        if (!isActive()) {
            ...
            return new InitiateResult(mock, null,
                    "/wallet/paypal-return?token=" + mock + "&mock=1",
                    null, null, null, Map.of("mock", true));
        }
        String token = fetchAccessToken();
        BigDecimal amount = BigDecimal.valueOf(p.getAmountUsdCents()).divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP);
        ...
        if (isOrder) purchase.put("invoice_id", p.getOrderId().toString());

        Map<String, Object> body = Map.of(
                "intent", "CAPTURE",
                "purchase_units", List.of(purchase),
                "application_context", Map.of(
                        "brand_name", "NX036 Dropshipping (" + platformEnv + ")",
                        "user_action", "PAY_NOW",
        ...
        Map<String, Object> resp = (Map<String, Object>) webClientBuilder.build()
                .post().uri(baseUrl + "/v2/checkout/orders")
                .header("Authorization", "Bearer " + token)
                .header("PayPal-Request-Id", p.getIdempotencyKey() != null ? p.getIdempotencyKey() : p.getId().toString())
        ...
        if (links != null) {
            for (Map<String, Object> link : links) {
                if ("approve".equals(link.get("rel"))) { approveUrl = String.valueOf(link.get("href")); break; }
            }
        }
    ...
    public Map<String, Object> capture(String paypalOrderId) {
        if (!isActive()) return Map.of("status", "COMPLETED", "mock", true);
        String token = fetchAccessToken();
        @SuppressWarnings("unchecked")
        Map<String, Object> resp = (Map<String, Object>) webClientBuilder.build()
                .post().uri(baseUrl + "/v2/checkout/orders/" + paypalOrderId + "/capture")
                .header("Authorization", "Bearer " + token)
                .header("Content-Type", "application/json")
                .bodyValue(Map.of())
                .retrieve().bodyToMono(Map.class).timeout(Duration.ofSeconds(20)).block();
    ...
    private String fetchAccessToken() {
        ...
        @SuppressWarnings("unchecked")
        Map<String, Object> body = (Map<String, Object>) webClientBuilder.build()
                .post().uri(baseUrl + "/v1/oauth2/token")
