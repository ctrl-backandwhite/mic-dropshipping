    ...
    public SecurityFilterChain bffFilterChain(HttpSecurity http) throws Exception {
        http.securityMatcher("/api/admin/**", "/api/storefront/**", "/api/me/**", "/api/auth/**", "/api/webhooks/**")
                ...
                .csrf(csrf -> csrf.csrfTokenRepository(CookieCsrfTokenRepository.withHttpOnlyFalse())
                        ...
                        .ignoringRequestMatchers("/api/auth/login", "/api/auth/register", "/api/auth/activate",
                                ...
                                "/api/storefront/affiliate/track", "/api/storefront/newsletter/subscribe",
                ...
                .authorizeHttpRequests(reg -> reg.requestMatchers(HttpMethod.OPTIONS, "/**").permitAll()
                        .requestMatchers("/api/auth/login", "/api/auth/register", "/api/auth/activate",
                                "/api/auth/password-reset/**", "/api/webhooks/**")
                        .permitAll().requestMatchers(HttpMethod.GET, "/api/storefront/**").permitAll()
