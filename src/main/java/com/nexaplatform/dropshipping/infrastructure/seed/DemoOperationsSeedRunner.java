...
public class DemoOperationsSeedRunner {
    ...
    public void seed() {
        ...
        if (operators.size() < 5) seedOperators(5);
        ...
        List<UserEntity> demoCustomers = customers.stream()
                .filter(u -> u.getEmail() != null && u.getEmail().contains("@demo.nx036.local"))
                .toList();
        List<UserEntity> demoPartners = partners.stream()
                .filter(u -> u.getEmail() != null && u.getEmail().contains("@partners.nx036.local"))
                .toList();

        if (addressRepository.count() < 50) seedAddressesFor(demoCustomers, demoPartners);
        if (paymentRepository.count() < 50) seedWalletDeposits(demoCustomers, demoPartners);
        if (orderRepository.count() < 30) seedOrders(demoCustomers, 60);
        if (subscriptionRepository.count() < 5) seedSubscriptions(demoPartners);
        if (priceRuleRepository.count() < 5) seedPriceRules();
        if (specRepository.count() < 50) seedSpecifications();
        if (attrRepository.count() < 50) seedAttributesAndTags();
        if (zoneRepository.count() < 20) seedShipping();
        seedProductExtras();
        seedExtraCategories();
        seedProductFlagsAndVideos();
        if (historyRepository.count() < 50) seedPriceHistory();
        if (agentProfileRepo.count() == 0) seedAgents();
        if (courseRepo.count() == 0) seedCourses();
        if (mentorProfileRepo.count() == 0) seedMentors(demoPartners);
        if (warehouseRepo.count() == 0) seedWarehouses();
        if (adTrendRepo.count() == 0) seedAdTrends();
        if (notificationRepo.count() < 20) seedNotifications(demoCustomers);
        if (shopConnectionRepo.count() == 0) seedShopConnections(demoCustomers);
    ...
    private static int countDemo(List<UserEntity> us) {
        return (int) us.stream()
                .filter(u -> u.getEmail() != null && u.getEmail().contains("@demo.nx036.local")
                          || u.getEmail() != null && u.getEmail().contains("@partners.nx036.local"))
                .count();
    }
    ...
    private List<UserEntity> seedCustomers(int n) {
        String[][] persons = {
                {"María",    "García",    "ES"}, {"Carlos",  "López",     "MX"},
                {"Sofía",    "Martínez",  "AR"}, {"Diego",   "Rodríguez", "CO"},
                {"Lucía",    "Sánchez",   "CL"}, {"Mateo",   "Fernández", "ES"},
                {"Camila",   "González",  "PE"}, {"Sebastián","Pérez",    "ES"},
                {"Valentina","Hernández", "MX"}, {"Daniela", "Díaz",      "BR"},
                {"James",    "Smith",     "US"}, {"Olivia",  "Brown",     "GB"},
                {"Liam",     "Wilson",    "CA"}, {"Emma",    "Anderson",  "AU"},
                {"Noah",     "Thomas",    "US"}, {"Ava",     "Taylor",    "GB"},
                {"Ethan",    "Moore",     "IE"}, {"Mia",     "Jackson",   "NZ"},
                {"Lucas",    "White",     "US"}, {"Charlotte","Harris",   "GB"},
                {"João",     "Silva",     "BR"}, {"Beatriz", "Santos",    "PT"},
                {"Pedro",    "Costa",     "PT"}, {"Ana",     "Almeida",   "BR"},
                {"Rafael",   "Oliveira",  "BR"}, {"Mariana", "Pereira",   "PT"},
                {"明",       "李",         "CN"}, {"芳",      "王",         "CN"},
                {"伟",       "张",         "CN"}, {"芳",      "刘",         "HK"},
                {"敏",       "陈",         "SG"}, {"娜",      "杨",         "CN"},
                {"Hiroshi",  "Tanaka",    "JP"}, {"Sakura",  "Yamamoto",  "JP"},
                {"Min-jun",  "Kim",       "KR"}, {"Seo-yeon","Park",      "KR"},
                {"Arjun",    "Patel",     "IN"}, {"Priya",   "Sharma",    "IN"},
                {"Rohan",    "Singh",     "IN"}, {"Aisha",   "Khan",      "AE"},
                {"Hugo",     "Bernard",   "FR"}, {"Camille", "Dubois",    "FR"},
                {"Lukas",    "Müller",    "DE"}, {"Lena",    "Schmidt",   "DE"},
                {"Giulia",   "Rossi",     "IT"}, {"Lorenzo", "Bianchi",   "IT"},
                {"Jan",      "Kowalski",  "PL"}, {"Eva",     "Novak",     "PL"},
                {"Erik",     "Eriksson",  "SE"}, {"Ingrid",  "Olsen",     "NO"},
        };
        ...
        for (int i = 0; i < Math.min(n, persons.length); i++) {
            ...
            UserEntity u = UserEntity.builder()
                    .email(email)
                    .passwordHash(passwordEncoder.encode("Demo!Customer2026"))
                    .role(UserRole.USER)
                    .active(true)
                    .displayName(p[0] + " " + p[1])
                    .country(p[2])
    ...
    private List<UserEntity> seedPartners(int n) {
        String[][] shops = {
                {"trendypicks-mx",     "TrendyPicks MX",        "MX", "Shopify"},
                ...
                {"buenosaires-mart",   "BA Mart",               "AR", "Shopify"},
                {"london-trends",      "London Trends",         "GB", "Shopify"},
                {"berlin-uptown",      "Berlin Uptown",         "DE", "WooCommerce"},
                {"paris-pop",          "Paris Pop",             "FR", "Shopify"},
                {"toronto-vault",      "Toronto Vault",         "CA", "Shopify"},
                {"miami-imports",      "Miami Imports",         "US", "BigCommerce"},
                {"lisbon-loop",        "Lisbon Loop",           "PT", "WooCommerce"},
                {"tokyo-curated",      "Tokyo Curated",         "JP", "Custom"},
                {"seoul-aesthetic",    "Seoul Aesthetic",       "KR", "Cafe24"},
                {"sydney-supply",      "Sydney Supply",         "AU", "Shopify"},
                {"dubai-prime",        "Dubai Prime",           "AE", "Shopify"},
        };
        List<UserEntity> out = new ArrayList<>();
        for (int i = 0; i < Math.min(n, shops.length); i++) {
            ...
            UserEntity u = UserEntity.builder()
                    .email(email)
                    .passwordHash(passwordEncoder.encode("Demo!Partner2026"))
                    .role(UserRole.PARTNER)
                    .active(true)
                    .displayName(s[1])
                    .companyName(s[1] + " · " + s[3])
                    .country(s[2])
                    .language(languageForCountry(s[2]))
                    .build();
            out.add(userRepository.save(u));
        }
        out.forEach(u -> walletUseCase.getOrCreate(u.getId()));
    ...
    private void seedOperators(int n) {
        String[][] ops = {
                {"ops.maria",   "María Ops",   "ES"},
                {"ops.carlos",  "Carlos Ops",  "MX"},
                {"ops.lin",     "Lin Ops",     "CN"},
                {"ops.priya",   "Priya Ops",   "IN"},
                {"ops.lucas",   "Lucas Ops",   "BR"},
        };
        for (int i = 0; i < Math.min(n, ops.length); i++) {
            ...
            userRepository.save(UserEntity.builder()
                    .email(email)
                    .passwordHash(passwordEncoder.encode("Demo!Operator2026"))
                    .role(UserRole.OPERATOR)
                    .active(true)
                    .displayName(o[1])
                    .country(o[2])
                    .language(languageForCountry(o[2]))
                    .build());
        }
    }
    ...
    private void seedAddressesFor(List<UserEntity> customers, List<UserEntity> partners) {
        int created = 0;
        for (UserEntity u : customers) created += seedAddresses(u, 1 + rnd.nextInt(3));
        for (UserEntity u : partners)  created += seedAddresses(u, 1 + rnd.nextInt(2));
        log.info("Seeded {} addresses", created);
    }

    private int seedAddresses(UserEntity u, int count) {
        String[] cities = citiesFor(u.getCountry());
        if (cities.length == 0) cities = new String[]{ "Capital" };
        String[] labels = { "Casa", "Oficina", "Casa de mis padres", "Almacén" };
        int created = 0;
        for (int i = 0; i < count; i++) {
            String city = cities[rnd.nextInt(cities.length)];
            UserAddressEntity a = UserAddressEntity.builder()
                    .user(u)
                    ...
                    .city(city)
                    .state(stateFor(u.getCountry(), city))
                    .postalCode(postalFor(u.getCountry()))
                    .country(u.getCountry() != null ? u.getCountry() : "US")
                    .isDefault(i == 0)
                    .build();
    ...
    private void createDeposit(UserEntity u, long cents, PaymentMethod method) {
        ...
        PaymentEntity p = PaymentEntity.builder()
                ...
                .providerRef("seed_" + UUID.randomUUID().toString().substring(0, 12))
    ...
    private List<CustomerOrderEntity> seedOrders(List<UserEntity> customers, int count) {
        ...
        String[] noteVariants = {
                ...
                "Dirección secundaria — llamar antes",
                "Regalo, sin recibo en la caja",
                "Entrega después de las 18:00",
        ...
        for (int i = 0; i < count; i++) {
            ...
            List<UserAddressEntity> addrs = addressRepository.findByUser_IdOrderByIsDefaultDescCreatedAtDesc(customer.getId());
            if (addrs.isEmpty()) continue;
            UserAddressEntity src = addrs.get(0);

            AddressEntity ship = orderAddressRepository.save(AddressEntity.builder()
                    .fullName(src.getFullName()).phone(src.getPhone()).email(customer.getEmail())
                    .line1(src.getLine1()).line2(src.getLine2()).city(src.getCity())
                    .state(src.getState()).postalCode(src.getPostalCode()).country(src.getCountry())
                    .build());

            CustomerOrderEntity o = CustomerOrderEntity.builder()
                    .orderNumber("NX-DEMO-" + String.format("%05d", 10000 + i))
                    .userId(customer.getId())
                    .externalOrderId("DEMO-" + (10000 + i))
                    .shippingAddress(ship)
                    .billingAddress(ship)
                    .status(OrderStatus.PENDING)
                    .currency("USD")
                    .subtotalCents(0).shippingCents(0).taxCents(0).totalCents(0)
                    .notes(noteVariants[rnd.nextInt(noteVariants.length)])
                    .placedAt(Instant.now().minus(rnd.nextInt(120), ChronoUnit.DAYS))
                    .build();
            ...
            for (int j = 0; j < itemCount; j++) {
                ...
                int qty = qtyBucket < 70 ? 1 + rnd.nextInt(3)
                        : qtyBucket < 90 ? 4 + rnd.nextInt(7)
                        : 11 + rnd.nextInt(40);
                ...
                if (baseUnit < 100) baseUnit = 200 + rnd.nextInt(15000); // 2..152 USD
                ...
                String img = prod.getImages() != null && !prod.getImages().isEmpty()
                        ? prod.getImages().get(0).getSourceUrl() : null;
                OrderItemEntity it = OrderItemEntity.builder()
                        .order(o)
                        .product(prod)
                        .titleSnapshot(prod.getTitleZh())
                        .imageUrlSnapshot(img)
                        .skuSnapshot(prod.getExternalId())
                        .unitPriceCents(unitCents)
                        .costCents((int) Math.round(unitCents * 0.65))
                        .quantity(qty)
            ...
            if (status == OrderStatus.PAID || status == OrderStatus.FORWARDED
    ...
    private void applyStatusTimeline(CustomerOrderEntity o, OrderStatus target) {
        ...
        switch (target) {
            ...
            default -> { /* PENDING / AWAITING_PAYMENT / PAID — no extra timestamps */ }
        }
    }
    ...
    private void seedSubscriptions(List<UserEntity> partners) {
        ...
        SubscriptionPlanEntity paid = plans.stream()
                .filter(p -> p.getPriceMonthlyCents() > 0)
                .findFirst().orElse(plans.get(0));
        SubscriptionPlanEntity free = plans.stream()
                .filter(p -> p.getPriceMonthlyCents() == 0)
                .findFirst().orElse(plans.get(0));
        ...
        for (int i = 0; i < partners.size(); i++) {
            ...
            if (!subscriptionRepository.findByUserId(p.getId()).isEmpty()) continue;
            SubscriptionStatus st = statuses[i % statuses.length];
            Instant now = Instant.now();
            CustomerSubscriptionEntity s = CustomerSubscriptionEntity.builder()
                    .user(p)
                    .plan(i % 3 == 0 ? free : paid)
                    .status(st)
                    .billingPeriod(i % 2 == 0 ? "MONTH" : "YEAR")
                    ...
                    .trialEndsAt(st == SubscriptionStatus.TRIALING ? now.plus(7, ChronoUnit.DAYS) : null)
                    .build();
    ...
    private void seedPriceRules() {
        if (priceRuleRepository.count() >= 5) return;
        ...
        boolean hasGlobalDefault = priceRuleRepository.findAll().stream()
                .anyMatch(r -> r.getScope() == PriceRuleScope.GLOBAL
                        && r.getMarginType() == MarginType.PERCENTAGE
                        && r.getScopeId() == null);
        if (!hasGlobalDefault) {
            rules.add(rule(PriceRuleScope.GLOBAL, null, MarginType.PERCENTAGE, "35.00", 100,
                    "Margen global por defecto (35%)"));
        }
        Map<String, BigDecimal> catMargins = Map.of(
                "consumer-electronics", new BigDecimal("28.00"),
                "fashion-apparel",      new BigDecimal("45.00"),
                "home-kitchen",         new BigDecimal("38.00"),
                "beauty-personal-care", new BigDecimal("52.00"),
                "sports-outdoors",      new BigDecimal("40.00"),
                "toys-gifts",           new BigDecimal("48.00"));
        catMargins.forEach((slug, value) -> categoryRepository.findBySlug(slug).ifPresent(cat -> {
            ...
            String label = cat.getTranslations() == null ? null : cat.getTranslations().stream()
                    .filter(tr -> "es".equals(tr.getLanguage()) || "en".equals(tr.getLanguage()))
                    .map(tr -> tr.getName())
                    .filter(n -> n != null && !n.isBlank())
                    .findFirst().orElse(null);
            if (label == null || label.isBlank()) label = cat.getNameZh();
            if (label == null || label.isBlank()) label = cat.getSlug();
            rules.add(rule(PriceRuleScope.CATEGORY, cat.getId(), MarginType.PERCENTAGE,
                    value.toPlainString(), 200, "Margen categoría — " + label));
        }));
        supplierRepository.findAll().stream().limit(3).forEach(s ->
                rules.add(rule(PriceRuleScope.SUPPLIER, s.getId(), MarginType.PERCENTAGE,
                        "30.00", 300, "Proveedor preferido — " + s.getName())));
    ...
    private PriceRuleEntity rule(PriceRuleScope scope, UUID scopeId, MarginType type,
                                 String value, int position, String description) {
        return PriceRuleEntity.builder()
                .scope(scope).scopeId(scopeId)
                .marginType(type).marginValue(new BigDecimal(value))
                .active(true).position(position).description(description)
                .build();
    }
    ...
    private void seedSpecifications() {
        ...
        for (ProductEntity p : products) {
            ...
            int pos = 0;
            for (String[] kv : es) {
                specRepository.save(ProductSpecificationEntity.builder()
                        .product(p).locale("es").specKey(kv[0]).specValue(kv[1]).position(pos++).build());
                created++;
            }
            pos = 0;
            for (String[] kv : en) {
                specRepository.save(ProductSpecificationEntity.builder()
                        .product(p).locale("en").specKey(kv[0]).specValue(kv[1]).position(pos++).build());
                created++;
            }
        }
    ...
    private List<String[]> specsFor(ProductEntity p, String lang) {
        ...
        if ("consumer-electronics".equals(category)) {
            ...
            out.add(new String[]{ es ? "Cuidado" : "Care", es ? "Lavar a 30°C, no plancha directa" : "Machine wash 30°C" });
    ...
    private void seedAttributesAndTags() {
        String[][] catTagPool = {
                {"consumer-electronics", "wireless,bluetooth,tech,gadget,charger,smart"},
        ...
        for (String[] e : catTagPool) tagsByCat.put(e[0], e[1].split(","));
        ...
        for (ProductEntity p : products) {
            ...
            // attributes
            attrRepository.save(ProductAttributeEntity.builder().product(p)
                    .attrKey("brand").attrValue(p.getBrand() != null ? p.getBrand() : "NX036").build());
            attrRepository.save(ProductAttributeEntity.builder().product(p)
                    .attrKey("origin").attrValue("CN").build());
            attrRepository.save(ProductAttributeEntity.builder().product(p)
                    .attrKey("season").attrValue(seasons[rnd.nextInt(seasons.length)]).build());
            attrRepository.save(ProductAttributeEntity.builder().product(p)
                    .attrKey("age_group").attrValue(ageGroups[rnd.nextInt(ageGroups.length)]).build());
            attrRepository.save(ProductAttributeEntity.builder().product(p)
                    .attrKey("category").attrValue(slug).build());
            aCount += 5;
            ...
            while (picks.size() < n) picks.add(pool[rnd.nextInt(pool.length)]);
    ...
    private void seedShipping() {
        String[] countries = { "US", "ES", "MX", "BR", "GB", "DE", "FR", "IT", "CA", "AU",
                               "JP", "KR", "SG", "AE", "CN", "HK", "AR", "CL", "CO", "PE",
                               "PT", "NL", "PL", "SE", "NO" };
        ...
        for (SupplierEntity s : suppliers) {
            for (String c : countries) {
                zoneRepository.save(ShippingZoneEntity.builder()
                        .supplier(s).countryCode(c).region(regionOf(c)).active(true).build());
                zCount++;
    ...
    private ShippingRateEntity rate(SupplierEntity s, String country, String method, String carrier,
                                    int dMin, int dMax, int baseCents, int perKgCents, int maxGrams) {
        return ShippingRateEntity.builder()
                .supplier(s).countryCode(country).method(method).carrier(carrier)
                .transitDaysMin(dMin).transitDaysMax(dMax)
                .baseCents(baseCents).perKgCents(perKgCents)
    ...
    private void seedProductExtras() {
        ...
        int updated = 0;
        for (ProductEntity p : products) {
            if (p.getLeadTimeDays() != null && p.getCertifications() != null) continue;
            if (p.getLeadTimeDays() == null)      p.setLeadTimeDays(2 + rnd.nextInt(8));
            if (p.getWarrantyMonths() == null)    p.setWarrantyMonths(3 + rnd.nextInt(22));
            if (p.getReturnPolicyDays() == null)  p.setReturnPolicyDays(rnd.nextInt(2) == 0 ? 14 : 30);
            if (p.getCountryOfOrigin() == null)   p.setCountryOfOrigin("CN");
    ...
    private void seedExtraCategories() {
        ...
        Object[][] extras = {
                ...
                { "office-supplies",  "20008", "办公用品",  8, "pen",        "Oficina y papelería",   "Office & Stationery",     "Escritório e papelaria" },
                ...
                { "jewelry-watches",  "20011", "珠宝手表", 11, "gem",        "Joyería y relojes",     "Jewelry & Watches",       "Joias e relógios" },
                { "garden-outdoor",   "20012", "园艺户外", 12, "leaf",       "Jardín y exterior",     "Garden & Outdoor",        "Jardim e exterior" },
                { "baby-maternity",   "20013", "母婴用品", 13, "baby",       "Bebés y maternidad",    "Baby & Maternity",        "Bebês e maternidade" },
                { "lighting",         "20014", "灯饰照明", 14, "lightbulb",  "Iluminación",           "Lighting",                "Iluminação" },
        };
        int created = 0;
        for (Object[] e : extras) {
            String slug = (String) e[0];
            if (categoryRepository.findBySlug(slug).isPresent()) continue;
        ...
        if (created > 0) log.info("Seeded {} extra root categories (total now {})", created, categoryRepository.count());
    }
    ...
    private void seedProductFlagsAndVideos() {
        ...
        for (ProductEntity p : products) {
            ...
            if (p.getShipFrom() == null) {
                p.setShipFrom(shipFromPool[rnd.nextInt(shipFromPool.length)]); changed = true;
            }
            if (p.getFreeShipping() == null) {
                p.setFreeShipping(rnd.nextInt(3) == 0); changed = true;     // ~33% free shipping
            }
            if (p.getSelfPickup() == null) {
                p.setSelfPickup(rnd.nextInt(8) == 0); changed = true;       // ~12% self pickup
            }
            if (p.getHasVideo() == null) {
                p.setHasVideo(rnd.nextInt(4) == 0); changed = true;         // ~25% have video
            }
            ...
            if (p.getInventoryCount() == null) {
                p.setInventoryCount(50 + rnd.nextInt(9950)); changed = true;
            }
            if (changed) { productRepository.save(p); updated++; }
        }
        if (updated > 0) log.info("Filled ship-from / free-shipping / video / inventory on {} products", updated);
    }
    ...
    private void seedPriceHistory() {
        ...
        int created = 0;
        for (ProductEntity p : products) {
            if (p.getBasePrice() == null) continue;
            ...
            for (java.time.LocalDate d = from; !d.isAfter(today); d = d.plusDays(1)) {
                ...
                historyRepository.save(ProductHistoryEntity.builder()
                        ...
                        .stock(Math.max(0, (int) Math.round(baseStock * stockNoise)))
                        .build());
                created++;
            }
        }
    ...
    private void seedAgents() {
        Object[][] agents = {
                {"Liang Chen",      "STRATEGIC", "10 years sourcing electronics from Shenzhen", List.of("en","zh","es"), 98.2, 4.7,  4.9, 412},
                {"María Pérez",     "SENIOR",    "Fashion & beauty supplier discovery",         List.of("es","en"),      94.8, 7.2,  4.8, 268},
                {"Ravi Sharma",     "SENIOR",    "Tools, hardware and auto parts negotiator",   List.of("en","hi"),      92.1, 8.0,  4.7, 192},
                {"Sofía Almeida",   "MID",       "Bilingual coordinator for LATAM stores",      List.of("es","pt","en"), 88.5, 9.5,  4.6, 134},
                ...
                {"Camille Dubois",  "STRATEGIC", "Premium beauty + EU compliance",              List.of("fr","en"),      96.5, 5.1,  4.85, 318},
        };
        for (Object[] a : agents) {
            @SuppressWarnings("unchecked")
            List<String> langs = (List<String>) a[3];
            agentProfileRepo.save(com.nexaplatform.dropshipping.infrastructure.persistence.entity.AgentProfileEntity.builder()
                    .displayName((String) a[0]).tier((String) a[1]).bio((String) a[2])
                    .languages(langs)
                    .successRate(new java.math.BigDecimal(a[4].toString()))
                    .avgResponseHours(new java.math.BigDecimal(a[5].toString()))
                    .satisfaction(new java.math.BigDecimal(a[6].toString()))
                    .completedJobs((int) a[7])
                    .hourlyRateUsdCents(2500 + rnd.nextInt(7500))
                    .active(true).build());
        }
    ...
    private void seedCourses() {
        Object[][] courses = {
                {"intro-dropshipping",    "Intro al Dropshipping en NX036", "Pilares del negocio y primeros 30 días.",    "Jesus Finol", 45,  "es", "BEGINNER"},
                {"sourcing-china",        "Sourcing efectivo en China",     "Cómo evaluar fábricas y MOQs realistas.",    "Liang Chen",  60,  "es", "INTERMEDIATE"},
                {"shopify-integration",   "Integración Shopify paso a paso","Conecta tu tienda y publica 50 productos.",  "María Pérez", 40,  "es", "BEGINNER"},
                {"winning-products",      "Hallar Winning Products",        "Uso de Intelligence + Ad Trends.",           "Tom Wilson",  35,  "es", "INTERMEDIATE"},
                {"shipping-fundamentals", "Logística internacional 101",    "STANDARD vs EXPRESS vs AIR vs SEA.",          "Sofía Almeida", 30,"es","BEGINNER"},
                {"pod-mastery",           "Print On Demand desde cero",     "Editor, mockups, IA y fulfillment POD.",     "Lin Zhao",    50,  "es", "INTERMEDIATE"},
                {"branding-odm",          "ODM y branding para tu marca",   "Cuándo invertir en custom packaging.",       "Camille Dubois", 40, "es", "ADVANCED"},
                {"intro-en",              "Intro to dropshipping on NX036", "Business pillars and your first 30 days.",   "Jesus Finol", 45,  "en", "BEGINNER"},
                {"sourcing-en",           "Effective sourcing from China",  "Evaluate factories and realistic MOQs.",     "Liang Chen",  60,  "en", "INTERMEDIATE"},
                {"shopify-en",            "Shopify integration step by step","Connect your shop and publish 50 products.","María Pérez", 40,  "en", "BEGINNER"},
                {"intro-pt",              "Introdução ao Dropshipping",     "Pilares do negócio e os primeiros 30 dias.", "Jesus Finol", 45,  "pt", "BEGINNER"},
                {"intro-zh",              "代发货入门",                       "业务基础与首个 30 天行动计划。",              "Jesus Finol", 45,  "zh", "BEGINNER"},
        };
        for (Object[] c : courses) {
            courseRepo.save(com.nexaplatform.dropshipping.infrastructure.persistence.entity.AcademyCourseEntity.builder()
                    .slug((String) c[0]).title((String) c[1]).description((String) c[2])
                    .instructor((String) c[3]).durationMinutes((int) c[4])
                    .locale((String) c[5]).level((String) c[6])
                    .coverUrl("https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=800")
                    .videoUrl("https://cdn.nx036.local/academy/" + c[0] + ".mp4")
    ...
    private void seedMentors(List<UserEntity> demoPartners) {
        ...
        for (int i = 0; i < Math.min(headlines.length, demoPartners.size()); i++) {
            ...
            mentorProfileRepo.save(com.nexaplatform.dropshipping.infrastructure.persistence.entity.MentorProfileEntity.builder()
                    .user(u).headline((String) headlines[i][0])
                    .expertise(expertise)
                    .languages(List.of("en","es"))
                    .hourlyRateUsdCents(6000 + rnd.nextInt(9000))
    ...
    private void seedWarehouses() {
        Object[][] whs = {
                {"CN-SHZ", "Shenzhen Hub",  "CN", "Shenzhen"},
                {"CN-YIW", "Yiwu Hub",      "CN", "Yiwu"},
                {"US-LAX", "Los Angeles",   "US", "Los Angeles"},
                {"US-NYC", "New York",      "US", "New York"},
                {"DE-FRA", "Frankfurt",     "DE", "Frankfurt"},
                {"GB-LON", "London",        "GB", "London"},
                {"PL-WAW", "Warsaw",        "PL", "Warsaw"},
                {"JP-TYO", "Tokyo",         "JP", "Tokyo"},
                {"ES-MAD", "Madrid",        "ES", "Madrid"},
                {"MY-KUL", "Kuala Lumpur",  "MY", "Kuala Lumpur"},
                {"MX-MEX", "Mexico City",   "MX", "Mexico City"},
                {"CA-YYZ", "Toronto",       "CA", "Toronto"},
        };
        List<com.nexaplatform.dropshipping.infrastructure.persistence.entity.WarehouseEntity> saved = new ArrayList<>();
        for (Object[] w : whs) {
            saved.add(warehouseRepo.save(com.nexaplatform.dropshipping.infrastructure.persistence.entity.WarehouseEntity.builder()
                    .code((String) w[0]).name((String) w[1]).country((String) w[2]).city((String) w[3])
                    .active(true).build()));
        }
        ...
        for (ProductEntity p : prods.subList(0, Math.min(30, prods.size()))) {
            ...
            for (int i = 0; i < 4 && i < saved.size(); i++) {
                pwsRepo.save(com.nexaplatform.dropshipping.infrastructure.persistence.entity.ProductWarehouseStockEntity.builder()
                        .product(p).warehouse(saved.get(i)).stock(20 + rnd.nextInt(2000)).build());
                stockRows++;
            }
        }
    ...
    private void seedAdTrends() {
        ...
        String[] hooks   = {
                "Vendo {} y ya facturo USD/día",
                "El producto que rompe en {} esta semana",
                "Cómo escalar {} con $50/día de ads",
                "{} viral en TikTok — checa el ratio CTR",
        ...
        for (int i = 0; i < 40 && i < prods.size() * 2; i++) {
            ...
            adTrendRepo.save(com.nexaplatform.dropshipping.infrastructure.persistence.entity.AdTrendEntity.builder()
                    ...
                    .impressions(10000L + (long) rnd.nextInt(900000))
                    .engagement(500L + (long) rnd.nextInt(40000))
                    .score(new java.math.BigDecimal(String.format(java.util.Locale.US, "%.3f", 0.5 + rnd.nextDouble() * 0.5)))
    ...
    private void seedNotifications(List<UserEntity> demoCustomers) {
        String[][] templates = {
                {"order.shipped", "Tu pedido fue enviado", "Sigue tu envío en /orders."},
        ...
        for (UserEntity u : demoCustomers.stream().limit(25).toList()) {
            for (int i = 0; i < 3; i++) {
                ...
                notificationRepo.save(com.nexaplatform.dropshipping.infrastructure.persistence.entity.NotificationEntity.builder()
                        .user(u).eventType(t[0]).title(t[1]).body(t[2])
    ...
    private void seedShopConnections(List<UserEntity> demoCustomers) {
        ...
        for (UserEntity u : demoCustomers) {
            ...
            String handle = (u.getDisplayName() != null ? u.getDisplayName().toLowerCase().replaceAll("[^a-z0-9]+", "-")
                    : "shop-" + Integer.toHexString(u.hashCode()))
                    + "-" + platform.toLowerCase().split("_")[0];
            shopConnectionRepo.save(com.nexaplatform.dropshipping.infrastructure.persistence.entity.ShopConnectionEntity.builder()
                    .user(u)
                    .platform(platform)
                    .shopHandle(handle)
                    .status("CONNECTED")
                    .lastSyncAt(java.time.Instant.now().minusSeconds(rnd.nextInt(86400)))
                    .build());
            created++;
        }
    ...
    private void seedProductBrandsAndReadyToShip() {
        ...
        for (ProductEntity p : productRepository.findAll()) {
            boolean changed = false;
            if (p.getBrandSelected() == null) { p.setBrandSelected(rnd.nextInt(3) == 0); changed = true; }
            if (p.getReadyToShip()  == null) { p.setReadyToShip(rnd.nextInt(4) == 0);  changed = true; }
            if (p.getPodEnabled()   == null) { p.setPodEnabled(rnd.nextInt(5) == 0);   changed = true; }
            if (changed) { productRepository.save(p); u++; }
        }
        if (u > 0) log.info("Flagged brand_selected/ready_to_ship/pod_enabled on {} products", u);
    }
    ...
    private PaymentMethod pickMethod() {
        ...
        if (r < 6) return PaymentMethod.CARD;
        if (r < 9) return PaymentMethod.PAYPAL;
        return PaymentMethod.USDT;
    }

    private String providerFor(PaymentMethod m) {
        return switch (m) { case CARD -> "stripe"; case PAYPAL -> "paypal"; case USDT -> "manual"; };
    }
    ...
    private static String[] citiesFor(String country) {
        if (country == null) return new String[]{ "Madrid" };
        return switch (country) {
