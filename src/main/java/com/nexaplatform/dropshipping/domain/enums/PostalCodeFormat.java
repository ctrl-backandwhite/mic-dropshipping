...
public enum PostalCodeFormat {
    ...
    /* ---- Unión Europea ---- */
    AT("\\d{4}", "1010"), BE("\\d{4}", "1000"), BG("\\d{4}", "1000"), CY("\\d{4}", "1010"), CZ("\\d{3} ?\\d{2}",
            "110 00"), DE("\\d{5}", "10115"), DK("\\d{4}", "1050"), EE("\\d{5}", "10111"), ES("\\d{5}",
                    "28001"), FI("\\d{5}", "00100"), FR("\\d{5}",
                            "75001"), GR("\\d{3} ?\\d{2}", "104 31"), HR("\\d{5}", "10000"), HU("\\d{4}", "1011"),
    /** Eircode: una letra, dos caracteres y cuatro alfanuméricos. */
    IE("[A-Z]\\d{2} ?[A-Z0-9]{4}", "D02 AF30"), IT("\\d{5}", "00184"), LT("(LT-)?\\d{5}", "01100"), LU("\\d{4}",
            "1009"), LV("(LV-)?\\d{4}", "1050"), MT("[A-Z]{3} ?\\d{4}", "VLT 1117"), NL("\\d{4} ?[A-Z]{2}",
                    "1012 AB"), PL("\\d{2}[- ]?\\d{3}", "00-001"), PT("\\d{4}[- ]?\\d{3}", "1000-001"), RO("\\d{6}",
                            "010011"), SE("\\d{3} ?\\d{2}",
                                    "111 29"), SI("(SI-)?\\d{4}", "1000"), SK("\\d{3} ?\\d{2}", "811 01"),

    /* ---- Resto de Europa ---- */
    CH("\\d{4}", "8001"),
    /** El código británico completo, con su parte exterior e interior. */
    GB("[A-Z]{1,2}\\d[A-Z\\d]? ?\\d[A-Z]{2}", "SW1A 1AA"), NO("\\d{4}", "0150"), RS("\\d{5}", "11000"), TR("\\d{5}",
            "34000"), UA("\\d{5}", "01001"),

    /* ---- América ---- */
    /** ZIP de cinco dígitos, con el sufijo de cuatro opcional. */
    US("\\d{5}([- ]?\\d{4})?", "10001"), CA("[A-Z]\\d[A-Z] ?\\d[A-Z]\\d", "K1A 0B1"), MX("\\d{5}", "01000"), BR(
            "\\d{5}[- ]?\\d{3}", "01001-000"), AR("[A-Z]?\\d{4}[A-Z]{0,3}", "C1425"), CL("\\d{7}",
                    "8320000"), CO("\\d{6}", "110111"), PE("\\d{5}", "15001"), UY("\\d{5}", "11000"),

    /* ---- Asia y Pacífico ---- */
    AU("\\d{4}", "2000"), NZ("\\d{4}", "1010"), JP("\\d{3}[- ]?\\d{4}", "100-0001"), KR("\\d{5}", "04524"), SG("\\d{6}",
            "018956"), MY("\\d{5}", "50450"), TH("\\d{5}", "10200"), ID("\\d{5}", "10110"), PH("\\d{4}",
                    "1000"), IN("\\d{6}", "110001"), IL("\\d{5,7}", "9103401"), ZA("\\d{4}", "0002");
